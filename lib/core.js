...
function countdownDisplay(deadline, now, mode='split') {
  const t=remaining(deadline,now);
  if(mode==='totalMinutes') return [String(t.days*1440+t.hours*60+t.minutes)];
  return [String(t.days).padStart(4,'0'),String(t.hours).padStart(2,'0'),String(t.minutes).padStart(2,'0')];
}
function countdownSeconds(deadline, now) {
  const end=Date.parse(deadline);
  if(!Number.isFinite(end)||!Number.isFinite(now)) throw new Error('Invalid countdown time');
  return Math.max(0,Math.ceil((end-now)/1000));
}
function countdownSnapshot(deadline, now, mode='split') {
  if(mode==='totalMinutes'){
    const totalSeconds=countdownSeconds(deadline,now);
    return {fields:[String(Math.floor(totalSeconds/60))],seconds:String(totalSeconds%60).padStart(2,'0')};
  }
  return {fields:countdownDisplay(deadline,now,mode),seconds:null};
}
function validateBundle(b) {
  ...
  for(const id of order) {
    ...
    for(const n of s.nodes) {
      ...
      if(n.type==='countdown'&&n.mode&&!['split','totalMinutes'].includes(n.mode)) throw new Error('Invalid countdown mode');
...
if(typeof module!=='undefined') module.exports={remaining,countdownDisplay,countdownSeconds,countdownSnapshot,validateBundle};
